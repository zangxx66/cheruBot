...
from cheru.utils import helper, chara, _pcr_data
import os
import json
...
sv_update = sv.on_command(cmd='update_pool', aliases={'更新卡池'}, permission=SUPERUSER, block=True)


@sv_update.handle()
async def update_pool_conf(bot: Bot, event: Event, state: dict):
    '''
    手动更新卡池时试用此命令
    '''
    status = await update_pool()
    if status == 0:
        await sv_update.finish('已是最新版本, 仍要更新卡池请使用【强制更新卡池】命令')
    elif status < 1000:
        await sv_update.finish(f'发生错误{status}')
    else:
        await sv_update.finish(f'更新完成, 当前卡池版本{status}')


sv_force_update = sv.on_command(cmd='force_update_pool', aliases={'强制更新卡池'}, permission=SUPERUSER, block=True)


@sv_force_update.handle()
async def force_update(bot: Bot, event: Event, state: dict):
    '''
    强制更新卡池
    '''
    status = await update_pool(force=True)
    if status == 0:
        await sv_force_update.finish(f'状态{status}')
    elif status < 1000:
        await sv_force_update.finish(f'发生错误{status}')
    else:
        await sv_force_update.finish(f'更新完成, 当前卡池版本{status}')
...
async def pool_scheduler():
    ...
    if status == 0:
        ...
        await bot.send_private_msg(seld_id=sid, user_id=master_id, message=msg, auto_escape=False)
    elif NOTICE:
