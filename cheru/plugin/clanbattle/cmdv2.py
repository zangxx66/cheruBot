...
from cheru.utils import helper, priv
...
def _check_admin(ctx: Event, tip: str = ''):
    if not priv.check_priv(ctx, priv.ADMIN):
        raise PermissionDeniedError(ERROR_PERMISSION_DENIED + tip)


@cb_cmd(('建会', '创建公会'), ArgParser(usage=USAGE_ADD_CLAN, arg_dict={
        'N': ArgHolder(tip='公会名'),
        'S': ArgHolder(tip='服务器地区', type=server_code)}))
async def add_clan(bot: Bot, ctx: Event, args: ParseResult):
    _check_admin(ctx)
...
async def list_clan(bot: Bot, ctx: Event, args: ParseResult):
    bm = BattleMaster(ctx.group_id)
    clans = bm.list_clan()
    if len(clans):
        clans = map(
...
@cb_cmd(('入会', '加入公会'), ArgParser(usage=USAGE_ADD_MEMBER, arg_dict={
        '': ArgHolder(tip='昵称', default=''),
        '@': ArgHolder(tip='qq号', type=int, default=0)}))
async def add_member(bot: Bot, ctx: Event, args: ParseResult):
    bm = BattleMaster(ctx.group_id)
    clan = _check_clan(bm)
...
async def list_member(bot: Bot, ctx: Event, args: ParseResult):
    bm = BattleMaster(ctx.group_id)
    clan = _check_clan(bm)
...
@cb_cmd(('退会', '退出公会'), ArgParser(usage='退会 (@qq)', arg_dict={
        '@': ArgHolder(tip='qq号', type=int, default=0)}))
async def del_member(bot: Bot, ctx: Event, args: ParseResult):
    bm = BattleMaster(ctx.group_id)
    uid = args['@'] or args.at or ctx.user_id
...
async def clear_member(bot: Bot, ctx: Event, args: ParseResult):
    bm = BattleMaster(ctx.group_id)
    clan = _check_clan(bm)
    _check_admin(ctx)
    msg = f"{clan['name']}已清空！" if bm.clear_member(
        1) else f"{clan['name']}已无成员"
    await bot.send(ctx, msg, at_sender=True)


@cb_cmd(('一键入会', '加入全部成员'), ArgParser('一键入会'))
async def batch_add_member(bot: Bot, ctx: Event, args: ParseResult):
    bm = BattleMaster(ctx.group_id)
    clan = _check_clan(bm)
    _check_admin(ctx)
    try:
    ...
    for m in mlist:
        if m['user_id'] != self_id:
            try:
                bm.add_member(m['user_id'], bm.group, m['card']
                              or m['nickname'] or str(m['user_id']), 1)
                succ += 1
...
async def process_challenge(bot: Bot, ctx: Event, ch: ParseResult):
    """
...
async def add_challenge(bot: Bot, ctx: Event, args: ParseResult):
...
@cb_cmd(('出尾刀', '收尾', '尾刀'), ArgParser(usage='出尾刀 (<伤害值>) (@<qq号>)', arg_dict={
    ...
    'R': ArgHolder(tip='周目数', type=round_code, default=0),
    'B': ArgHolder(tip='Boss编号', type=boss_code, default=0)}))
async def add_challenge_last(bot: Bot, ctx: Event, args: ParseResult):
    challenge = ParseResult({
        'round': args.R,
        'boss': args.B,
        'damage': args.get(''),
...
@cb_cmd(('出补时刀', '补时刀', '补时', '剩余刀'), ArgParser(usage='出补时刀 <伤害值> (@qq)', arg_dict={
    '': ArgHolder(tip='伤害值', type=damage_int),
    '@': ArgHolder(tip='qq号', type=int, default=0),
    'R': ArgHolder(tip='周目数', type=round_code, default=0),
    'B': ArgHolder(tip='Boss编号', type=boss_code, default=0)}))
async def add_challenge_ext(bot: Bot, ctx: Event, args: ParseResult):
    challenge = ParseResult({
        'round': args.R,
        'boss': args.B,
        'damage': args.get(''),
...
@cb_cmd('掉刀', ArgParser(usage='掉刀 (@qq)', arg_dict={
    ...
    'R': ArgHolder(tip='周目数', type=round_code, default=0),
    'B': ArgHolder(tip='Boss编号', type=boss_code, default=0)}))
async def add_challenge_timeout(bot: Bot, ctx: Event, args: ParseResult):
    challenge = ParseResult({
        'round': args.R,
        'boss': args.B,
        'damage': 0,
...
async def del_challenge(bot: Bot, ctx: Event, args: ParseResult):
    bm = BattleMaster(ctx.group_id)
    now = datetime.now()
    clan = _check_clan(bm)
...
# TODO 将预约信息转至数据库
SUBSCRIBE_PATH = os.path.expanduser('~/.cheru/clanbattle_sub/')
...
async def subscribe(bot: Bot, ctx: Event, args: ParseResult):
    bm = BattleMaster(ctx.group_id)
    uid = ctx.user_id
    _check_clan(bm)
...
@cb_cmd(('取消预约', '预约取消'), ArgParser(usage='取消预约 <Boss号>', arg_dict={
    '': ArgHolder(tip='Boss编号', type=boss_code)}))
async def unsubscribe(bot: Bot, ctx: Event, args: ParseResult):
    bm = BattleMaster(ctx.group_id)
    uid = ctx.user_id
    _check_clan(bm)
    _check_member(bm, uid, bm.group)
...
async def auto_unsubscribe(bot: Bot, ctx: Event, gid, uid, boss):
    sub = _load_sub(gid)
    slist = sub.get_sub_list(boss)
    if uid not in slist:
        return
...
async def list_subscribe(bot: Bot, ctx: Event, args: ParseResult):
    bm = BattleMaster(ctx.group_id)
    clan = _check_clan(bm)
    msg = [ f"\n{clan['name']}当前预约情况：" ]
    sub = _load_sub(bm.group)
...
@cb_cmd(('清空预约', '预约清空', '清理预约', '预约清理'), ArgParser('清空预约', arg_dict={
    '': ArgHolder(tip='Boss编号', type=boss_code)}))
async def clear_subscribe(bot: Bot, ctx: Event, args: ParseResult):
    bm = BattleMaster(ctx.group_id)
    clan = _check_clan(bm)
...
async def set_subscribe_limit(bot: Bot, ctx: Event, args: ParseResult):
    bm = BattleMaster(ctx.group_id)
    clan = _check_clan(bm)
...
async def add_sos(bot: Bot, ctx: Event, args: ParseResult):
    bm = BattleMaster(ctx.group_id)
    uid = ctx.user_id
    clan = _check_clan(bm)
    _check_member(bm, uid, bm.group)
...
async def list_sos(bot: Bot, ctx: Event, args: ParseResult):
    bm = BattleMaster(ctx.group_id)
    clan = _check_clan(bm)
    sub = _load_sub(bm.group)
    tree = sub.get_tree_list()
...
async def lock_boss(bot: Bot, ctx: Event, args: ParseResult):
    bm = BattleMaster(ctx.group_id)
    _check_clan(bm)
...
async def unlock_boss(bot: Bot, ctx: Event, args: ParseResult):
    bm = BattleMaster(ctx.group_id)
    _check_clan(bm)
    sub = _load_sub(bm.group)
    lock = sub.get_lock_info()
...
async def auto_unlock_boss(bot: Bot, ctx: Event, bm: BattleMaster):
    sub = _load_sub(bm.group)
    lock = sub.get_lock_info()
    if lock:
        uid, ts = lock[0]
...
async def show_progress(bot: Bot, ctx: Event, args: ParseResult):
    bm = BattleMaster(ctx.group_id)
    clan = _check_clan(bm)
...
async def stat_damage(bot: Bot, ctx: Event, args: ParseResult):
    bm = BattleMaster(ctx.group_id)
    now = datetime.now()
    clan = _check_clan(bm)
...
async def stat_score(bot: Bot, ctx: Event, args: ParseResult):
    bm = BattleMaster(ctx.group_id)
    now = datetime.now()
    clan = _check_clan(bm)
    yyyy, mm, _ = bm.get_yyyymmdd(now)
...
async def _do_show_remain(bot: Bot, ctx: Event, args: ParseResult, at_user: bool):
    bm = BattleMaster(ctx.group_id)
    clan = _check_clan(bm)
    if at_user:
...
@cb_cmd('查刀', ArgParser(usage='查刀', arg_dict={
        'D': ArgHolder(tip='日期差', type=int, default=0)}))
async def list_remain(bot: Bot, ctx: Event, args: ParseResult):
    await _do_show_remain(bot, ctx, args, at_user=False)
@cb_cmd('催刀', ArgParser(usage='催刀'))
async def urge_remain(bot: Bot, ctx: Event, args: ParseResult):
...
@cb_cmd('出刀记录', ArgParser(usage='出刀记录 (@qq)', arg_dict={
        '@': ArgHolder(tip='qq号', type=int, default=0),
        'D': ArgHolder(tip='日期差', type=int, default=0)}))
async def list_challenge(bot: Bot, ctx: Event, args: ParseResult):
    bm = BattleMaster(ctx.group_id)
    clan = _check_clan(bm)
